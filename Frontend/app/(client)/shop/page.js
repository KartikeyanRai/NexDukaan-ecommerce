...
// 2. Rename your main logic to "ShopContent" (internal component)
function ShopContent() {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  
  // This hook is what caused the build error without Suspense
  const searchParams = useSearchParams(); 
  const searchQuery = searchParams.get('q') || '';
...
// 3. EXPORT THE WRAPPED COMPONENT (This fixes the build error)
export default function ShopPage() {
  return (
    <Suspense fallback={<div className="text-center py-20 font-bold text-gray-400">Loading catalog...</div>}>
      <ShopContent />
    </Suspense>
  );
}
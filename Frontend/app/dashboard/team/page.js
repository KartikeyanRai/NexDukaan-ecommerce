...
//     // Get token for Authorization header logic (handled by API client usually, but good to ensure)
//     try {
//       await API.post('/auth/add-admin', formData);
//       setStatus({ type: 'success', message: `Successfully added ${formData.name} to the team!` });
//       setFormData({ name: '', email: '', password: '' }); // Reset form
//     } catch (err) {
//       setStatus({ type: 'error', message: err.response?.data?.message || "Failed to add admin" });
//     } finally {
//       setLoading(false);
//     }
//   };
...
//           <p className="text-gray-400 text-sm mt-1">Grant full store access to a partner</p>
//         </div>
...
//                   className="w-full pl-10 p-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 ring-green-500 outline-none transition"
...
//               type="submit" 
//               disabled={loading}
//               className="w-full py-4 bg-gray-900 text-white font-bold rounded-xl hover:bg-black transition shadow-lg disabled:opacity-70"
//             >
//               {loading ? 'Adding User...' : 'Add Team Member'}
//             </button>
//           </form>
//         </div>
//       </div>
